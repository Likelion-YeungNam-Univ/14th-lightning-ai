...
**대상은 국내 상장 주식(KOSPI·KOSDAQ) 전체 + 해외 화이트리스트 15개 내외다** (명세 v3에서 해외 축 추가). 화면의 1차 축은 **국내/해외 구분**이며, 구분이 탭 구성을 결정한다 — 국내: 유튜브·공시(DART)·규제(정책브리핑)·한국은행·Fed / 해외: 유튜브·공시(SEC)·규제(Federal Register)·Fed. 화이트리스트 밖 해외 종목은 검색에서 제외하고 `unsupported_overseas` 안내만.
...
flowchart LR
    ...
    subgraph BATCH["배치 경로 (매일 06:00 + 종목 추가 시)"]
        ...
        COL --> EXT["외부 API<br/>국내: KRX · DART · 정책브리핑 · ECOS<br/>해외: SEC EDGAR · Federal Register<br/>공통: FRED · YouTube"]
        COL --> AI["AI 가공 ai/<br/>요약·라벨·문장 (OpenAI)"]
        AI --> DB
        COL --> DB
    end
```

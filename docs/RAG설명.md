...
flowchart LR
    ...
    RL -- 통과 --> EX{"표제어 정확 일치?<br/>(사전 조회 우선)"}
    EX -- "일치 (유사도 1.0)" --> LLM
    EX -- 불일치 --> EMB["용어 임베딩<br/>text-embedding-3-small"]
    EMB --> VS["pgvector 코사인 검색<br/>top-3, 유사도 ≥ 0.45"]
...
- **유사도 하한은 왜 두나?** → 관련 없는 근거를 억지로 주입하면 오히려 오답을 유도한다.
  실측에서 사전에 없는 용어('공매도'는 2020년판에 미수록)에도 0.41짜리 무관 근거가 잡혀
  하한을 0.45로 올렸다. 미달이면 근거 없이 생성하되 "모르면 모른다고 답하라"로 방어한다.

- **왜 벡터 검색 전에 표제어 일치를 먼저 보나?** → 이 기능은 본질이 사전 조회다. 드래그한
  단어가 표제어와 정확히 일치하면('PER' → '주가수익비율(PER)') 그게 정답이며, 임베딩 호출
  비용도 아낀다. 일치가 없을 때만 의미 기반(벡터) 검색으로 넘어간다.

- **벡터 인덱스(HNSW·IVFFlat)는 왜 안 썼나?** → 지식베이스가 701행이라 전수 코사인 스캔이
  1ms 미만이고 재현율 100%다. 근사 인덱스(HNSW)는 수만~수백만 벡터에서 검색 시간을 줄이는
  대신 재현율을 조금 포기하는 트레이드오프라, 이 규모에서는 오히려 손해다. 뉴스 본문 등으로
  지식베이스가 수만 청크로 커지면 `CREATE INDEX ... USING hnsw (embedding vector_cosine_ops)`
  한 줄로 전환할 수 있게 pgvector를 선택해 뒀다 — "규모에 맞는 선택 + 확장 경로 확보"가 답변 포인트.

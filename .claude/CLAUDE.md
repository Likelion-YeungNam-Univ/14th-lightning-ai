...
1. **사용자 요청 경로에서 외부 API·LLM을 호출하지 않는다.** 수집(F-4)과 생성(F-5)은 배치/온디맨드에서 끝내고 DB에 적재하며, 조회 API는 DB만 읽는다. 유일한 예외는 용어 풀이 `POST /terms/explain`과 그 SSE 변형 `GET /terms/explain/stream`(F-5.4, LLM 호출 + 레이트리밋 필수 — 같은 F-항목).
...
6. **구현을 마치면 "무엇이 깨질 수 있는가"(엣지 케이스·경합 지점)를 먼저 나열하고, 그 목록을 테스트로 검증한 뒤, 테스트 결과를 표/목록으로 간단히 정리해 보고한다** (팀 합의 작업 방식)

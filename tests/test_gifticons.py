...
def _grant_points_for_client(client, amount: int = 20_000) -> None:
...
def test_gifticon_insufficient_points(client, login_env):
    _login(client)
    _grant_points_for_client(client, amount=10_000)  # 18,000P 미만
    r = client.post("/me/gifticons")
...
def test_gifticon_exchange_success(client, login_env):
    _login(client)
    _grant_points_for_client(client, amount=20_000)
    r = client.post("/me/gifticons")
    assert r.status_code == 200
    body = r.json()
    assert body["points_used"] == 18_000
...
def test_gifticon_monthly_limit(client, login_env):
    _login(client)
    _grant_points_for_client(client, amount=40_000)
